...
public class Placeholders {
    ...
    public static final String WIKI_MAIN_URL    = Placeholders.GITHUB_URL + "/wiki/";
    public static final String WIKI_ITEMS_URL   = Placeholders.WIKI_MAIN_URL + "ItemStack-Options";
    public static final String WIKI_TEXT_URL    = Placeholders.WIKI_MAIN_URL + "Text-Formation";
    public static final String WIKI_LANG_URL    = Placeholders.WIKI_MAIN_URL + "Language-Formation";
    ...
    // @formatter:on
    @NotNull
    public static UnaryOperator<String> forLocation(@NotNull final Location location) {
        return new PlaceholderMap().add(Placeholders.LOCATION_X, () -> NumberUtil.format(location.getX()))
                .add(Placeholders.LOCATION_Y, () -> NumberUtil.format(location.getY()))
                .add(Placeholders.LOCATION_Z, () -> NumberUtil.format(location.getZ()))
                .add(Placeholders.LOCATION_WORLD, () -> LocationUtil.getWorldName(location)).replacer();
    }

    @SuppressWarnings("deprecation")
    @NotNull
    public static UnaryOperator<String> forPlayer(@NotNull final Player player) {
        return new PlaceholderMap().add(Placeholders.PLAYER_NAME, player::getName)
                .add(Placeholders.PLAYER_DISPLAY_NAME, player::getDisplayName).replacer();
    }

    @NotNull
    public static UnaryOperator<String> forSender(@NotNull final CommandSender sender) {
        if (sender instanceof final Player player)
            return Placeholders.forPlayer(player);

        return new PlaceholderMap().add(Placeholders.PLAYER_NAME, sender::getName).add(Placeholders.PLAYER_DISPLAY_NAME, sender::getName)
                .replacer();
    }
}
