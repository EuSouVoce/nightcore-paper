...
public class CoreConfig {
    ...
    public static final ConfigValue<Boolean> MODERN_TEXT_PRECOMPILE_LANG = ConfigValue.create("ModernTextFormation.Precompile_Language",
            true, "When enabled, parses (deserializes) language messages to Spigot TextComponent(s) on plugin load.",
    ...
    public static final ConfigValue<Map<String, Color>> MODERN_TEXT_DEFAULT_COLORS = ConfigValue
            .forMap("ModernTextFormation.Default_Colors", (cfg, path, id) -> {
                try {
                    ...
                    .forEach((id, value) -> cfg.set(path + "." + id, "#" + Integer.toHexString(value.getRGB()).substring(2))), () -> {
    ...
    public static final ConfigValue<Long> MENU_CLICK_COOLDOWN = ConfigValue.create("Menu.Click_Cooldown", 150L,
            "Sets cooldown (in milliseconds) for player clicks in GUIs.", "[Default is 150ms]");

    public static final ConfigValue<Boolean> USER_DEBUG_ENABLED = ConfigValue.create("UserData.Debug", false,
            "Enables debug messages for user data management.", "[Default is false]");

    public static final ConfigValue<Integer> USER_CACHE_LIFETIME = ConfigValue.create("UserData.Cache.LifeTime", 300,
            "Sets how long (in seconds) user data will be cached for offline users",
            "until removed and needs to be loaded from the database again.", "[Default is 300 (5 minutes)]");
    ...
    public static final ConfigValue<Boolean> RESPECT_PLAYER_DISPLAYNAME = ConfigValue.create("Engine.Respect_Player_DisplayName", false,
            "Sets whether or not 'Player#getDisplayName' can be used to find & get players in addition to regular 'Player#getName'.",
            "This is useful if you want to use custom player nicknames in commands.", "(Works only for nightcore based plugins.)",
            "[Default is false]");
